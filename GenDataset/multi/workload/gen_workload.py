...
from index import Select
from index import SelectParser as sp
from index import DataLoader as data
...
def generate_workload_from_sql2(sql_path, database):
    wheres = []
    renames = []
    for sql in open(sql_path):
        wheres.append(sp.get_where(sql))
        renames.append(sp.get_tables(sql))
    return wheres,renames
