...
from GenDataset.multi.workload.gen_workload import generate_workload_from_sql2
...
from index.main import execute 
from index.test import index_query
...
def cal_vec(database, query, query_num, histo, standard_sample, sample_group, sample_feature, table_dict):
    ...
    for sample_name, sample_card in card.items():
        ...
        # 实验代码
        if sample_card == 0:
            sample_q_error1 = cal_q_error(gt_card, sample_card)
            print("sample_q_error1:",sample_q_error1)
            with open('sample_q_error1.txt', 'a') as file:
                # 写入 sample_q_error，并换行使每次写入在新的一行
                file.write(str(sample_q_error1) + '\n')
                print("sample_q_error2 已追加到 sample_q_error2.txt 文件中")
            
            sample_card = index_query(query_sql)
            sample_q_error2 = cal_q_error(gt_card, sample_card)
            print("sample_q_error:",sample_q_error1)
            with open('sample_q_error2.txt', 'a') as file:
                # 将 sample_q_error 转换为字符串并写入文件
                file.write(str(sample_q_error2)+'\n')
                print("sample_q_error1 已保存到 sample_q_error1.txt 文件中")
        record = {}
...
def gen_multi_dataset(dataset_name, workload_name, table_list, join_col, base_table, table_dict):
    ...
    # index_query
    # index_query_wheres,index_query_renames = generate_workload_from_sql2(sql_path, database)
