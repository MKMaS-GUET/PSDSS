...
class SamplingGroup(Estimator):
    def __init__(self, table: Table):
        ...
        num = 1
        # 计算离散化列的数量和实际列数，保证了不会尝试离散化比实际列数还多的列，同时也遵守空间预算的限制
    ...
    def select_irrelevant_col(self):
        ...
        return selected_col
    # 计算变异系数CV，选择CV较低的列,根据数据表中各列的变异程度选择一些相对稳定的列，以用于建立样本
    ...
    def query2(self, query,table):
        card_dict = {}
        dur_dict = {}
        for name, unit in self.name_to_sample_unit.items():
            card_dict[name], dur_dict[name] = unit.query2(query,table)
        return card_dict, dur_dict
...
class StratifiedSamplingUnit(object):
    ...
    def query2(self, query,table):
        ...
        dur = 0
        bitmap = np.ones(self.sample_num, dtype=bool)
        bitmap2 = np.ones(self.row_num, dtype=bool)
        df = self.sample
        for c, o, v in zip(columns, operators, values):
            dfc = df[c]
            start_stmp = time.time()
        ...
        # if card == 0:
        #     for c, o, v in zip(columns, operators, values):
        #         dfc_table = table.data[c]
        #         start_stmp = time.time()
        #         bitmap2 = OPS[o](dfc_table, v)
        #         table.data = table.data[bitmap2]
        #         bitmap2 = bitmap2[bitmap2]
        #     card =  bitmap2.sum()
        if card == 0:
            for c, o, v in zip(columns, operators, values):
                # dfc_table = table.data[c]
