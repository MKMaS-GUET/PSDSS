...
def cal_query_set(id, queries, query_num_offset, table, histo, standard_sample, sample_group, sample_feature, q: Queue):
    #  `rec_for_queryset` 用于存储每个查询的结果。
    #  `query_info_list` 用于收集每个查询的信息。
    #  `time_dict` 用于跟踪不同阶段的时间消耗，例如“gt_time”、“sg_time”等
    rec_for_queryset = []
    query_info_list = []
...
def cal_vec(table, query, query_num, histo, standard_sample, sample_group, sample_feature):
    ...
    # confidence interval
    # cards,card_interval  = standard_sample.query3(query) 
    ...
    card, cost = sample_group.query2(query,table)
    sg_time = (time.time() - start_stmp) / 60


    # generate features for query
    # 处理数据库查询并将其转换为不同的格式
    start_stmp = time.time()
